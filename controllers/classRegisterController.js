...
exports.cancelRegistration = async (req, res) => {
  try {
    ...
    if (tutors.length === 0) {
      return res.status(403).json({
        success: false,
        message: "Chỉ gia sư mới có thể hủy đăng ký nhận lớp",
      });
    }

    const tutorId = tutors[0].id;

    // Kiểm tra đăng ký tồn tại và thuộc về gia sư này
    const [registrations] = await db.query(
      "SELECT * FROM class_register WHERE id = ? AND tutor_id = ?",
      [registrationId, tutorId]
    );

    if (registrations.length === 0) {
      return res.status(404).json({
        success: false,
        message: "Không tìm thấy thông tin đăng ký hoặc bạn không có quyền",
      });
    }

    // Kiểm tra trạng thái đăng ký
    if (registrations[0].status === 'accepted') {
      return res.status(400).json({
        success: false,
        message: "Không thể hủy đăng ký đã được chấp nhận. Vui lòng liên hệ phụ huynh.",
      });
    }

    // Tiến hành xóa đăng ký
    await db.query(
      "DELETE FROM class_register WHERE id = ?",
      [registrationId]
    );

    res.status(200).json({
      success: true,
      message: "Hủy đăng ký nhận lớp thành công",
    });
  } catch (err) {
    console.error("Lỗi khi hủy đăng ký nhận lớp:", err);
    res.status(500).json({
      success: false,
      message: "Đã xảy ra lỗi khi hủy đăng ký nhận lớp",
    });
  }
}; 